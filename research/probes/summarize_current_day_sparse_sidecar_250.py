...
def main() -> None:
    ...
    no_takeover_hash_failures = []
    for suite, seed, parent, sidecar, _delta, _outcome, parent_days, sidecar_days in pairs:
        if int(sidecar.get("current_day_sparse_takeovers", "0")) == 0:
            if parent_days != sidecar_days or score(parent) != score(sidecar):
                no_takeover_hash_failures.append(f"{suite}:{seed}")
    ...
    gate = {
        ...
        "no_takeover_plan_hash_equivalence": not no_takeover_hash_failures,
    }
    gate["passed"] = all(gate.values())

    summary = {
        "experiment": "SCORE-CURRENT-DAY-SPARSE-SIDECAR-250",
        "phase": args.phase,
        "pairs": len(pairs),
        ...
        "no_takeover_hash_failures": no_takeover_hash_failures,
        "gate": gate,
    }
