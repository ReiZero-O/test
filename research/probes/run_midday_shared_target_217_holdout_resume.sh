#!/usr/bin/env bash
# SCORE-MIDDAY-SHARED-TARGET-217 sealed-holdout resume wrapper. Source,
# manifest and binary must remain frozen after the development gate.
# Usage: run_midday_shared_target_217_holdout_resume.sh <binary> <side:off|on> <logdir>
set -euo pipefail
binary="$1"
side="$2"
logdir="$3"
...
log="$logdir/SCORE-MIDDAY-SHARED-TARGET-217-holdout-causal-$side.log"
...
if [ ! -f "$log" ]; then
    {
        ...
        echo "binary_sha256=${BINARY_SHA256:?set BINARY_SHA256 to the frozen development binary hash}"
    } > "$log"
fi
...
run_tier stratified-easy 5010000 16 4 12 14
run_tier stratified-medium 5011000 16 4 12 18
run_tier stratified-hard 5012000 36 6 18 24
run_tier stratified-very-hard 5013000 40 8 18 24 30
