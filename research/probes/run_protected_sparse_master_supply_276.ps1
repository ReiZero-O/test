param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-PROTECTED-SPARSE-MASTER-SUPPLY-276.csv',
    [string]$Binary = 'build-research-276-msvc/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = 'BCA44D602CF6799F458127BA23D76794C58520C7896E67DC809751CF8C418FCE',
    [switch]$ValidateOnly,
    [switch]$Resume
)
...
$expectedManifestSha256 = '4F3F9A626945ED62B20D2606E475E5D98EF0B3E634A83C208C4898FCCD10AC34'
...
foreach ($row in $allRows) {
    if ($row.experiment_id -ne 'SCORE-PROTECTED-SPARSE-MASTER-SUPPLY-276' -or
...
if ($allRows.Count -ne 24 -or $caseCounts.development -ne 30 -or $caseCounts.holdout -ne 54) {
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-PROTECTED-SPARSE-MASTER-SUPPLY-276-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-PROTECTED-SPARSE-MASTER-SUPPLY-276,phase=${Phase},expected_results=${expectedResults}"
...
foreach ($line in Get-Content -LiteralPath $outputPath) {
    ...
    } elseif ($line -match '^result,version=causal-276,track=[^,]+,suite=([^,]+),.*seed=([0-9]+),') {
...
foreach ($row in $rows) {
    for ($offset=0; $offset -lt [int]$row.count; ++$offset) {
        ...
        $arguments = @(
            '--version','causal-276','--track','protected-sparse-master-supply-276',
            ...
            '--terminal-marginal-reservoir',1,'--midday-chain',1,
            '--midday-pair',0,'--midday-target-followup',1,
            '--protected-sparse-master-supply',1,
            '--public-window-probe-ms',$row.public_window_ms,
            '--checkpoint-closed-loop',1,'--day-details'
        )
        $caseOutput = @(& $binaryPath @arguments 2>&1)
        $exitCode = $LASTEXITCODE
        if ($exitCode -ne 0 -or @($caseOutput | Where-Object { $_ -match '^result,' }).Count -ne 1) {
            throw "$($row.suite)/${seed} failed (${exitCode}): $($caseOutput -join [Environment]::NewLine)"
        }
        @(
