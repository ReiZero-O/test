...
if ! mkdir "$lock_dir"; then
    echo "another 307 runner already owns $lock_dir" >&2
    exit 73
fi
...
run_seed() {
    ...
    "$binary" --manifest "$manifest" --split development --only-seed "$seed" --details \
        >"$partial" 2>"$stderr_partial"
    test "$(grep -c '^case,' "$partial")" -eq 1
    test "$(grep -c '^trace,' "$partial")" -eq 4
...
for seed in "${seeds[@]}"; do
    ...
    grep -E '^(case|trace),' "$output_dir/$seed.result" >>"$combined_partial"
done
test "$(grep -c '^case,' "$combined_partial")" -eq 4
test "$(grep -c '^trace,' "$combined_partial")" -eq 16
printf 'run_complete,cases=4,traces=16\n' >>"$combined_partial"
