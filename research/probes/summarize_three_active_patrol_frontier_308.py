...
def terminal_multiset(text: str) -> tuple[tuple[int, int], ...]:
    values = []
    for agent in text.split("|"):
        cell, fuel = agent.split("@", 1)
        values.append((int(cell), int(fuel)))
    return tuple(sorted(values))


def classify(row: dict[str, str]) -> str:
    if int(row["oracle_state_matches"]) == 0:
        return "terminal-state-supply"
    if row["state_selected"] == "1":
        return "state-selected"
    if row["state_certified"] == "1":
        return "certified-not-selected"
    if row["state_disposition"] == "not-shortlisted":
        return "frontier-not-shortlisted"
    return "frontier-not-certified"
...
def main() -> None:
    ...
    for seed in sorted(cases):
        ...
        causal = next(
            (
                ...
                or terminal_multiset(row["oracle_agents"])
                != terminal_multiset(row["head_agents"])
            ),
            None,
        )
        if causal is None:
            raise SystemExit(f"seed {seed} has no permutation-invariant divergence")
        day = int(causal["day"])
        classification = classify(causal)
        ...
        results.append(
            {
                ...
                "oracle_state_matches": int(causal["oracle_state_matches"]),
                "state_disposition": causal["state_disposition"],
                "state_certified": causal["state_certified"] == "1",
                "state_selected": causal["state_selected"] == "1",
                "state_lower": list(score(causal["state_lower"])),
                "state_upper": list(score(causal["state_upper"])),
                "selected_score": list(score(causal["selected_score"])),
                "selected_lower": list(score(causal["selected_lower"])),
                "selected_upper": list(score(causal["selected_upper"])),
    ...
    summary = {
        "experiment": "ATTR-THREE-ACTIVE-PATROL-FRONTIER-308",
