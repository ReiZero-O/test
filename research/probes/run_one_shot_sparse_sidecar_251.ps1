param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-ONE-SHOT-SPARSE-SIDECAR-251.csv',
    [string]$Binary = 'build-contract-fix/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = '5EEAE0F67770FC9AC3311E79801465DAE0EE927143A4ADE1599D546CBB613B7B',
    [switch]$Resume
)
...
$expectedManifestSha256 = '8A00B64171CB529ADF04AC230C348132F5849E07472DEF99CFBA375C0AF66811'
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-ONE-SHOT-SPARSE-SIDECAR-251-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-ONE-SHOT-SPARSE-SIDECAR-251,phase=${Phase},expected_pairs=${expectedPairs},expected_results=${expectedResults}"
...
foreach ($row in $rows) {
    for ($offset = 0; $offset -lt [int]$row.count; ++$offset) {
        ...
        foreach ($label in $labels) {
            ...
            $arguments = @(
                '--version', $label,
                '--track', 'one-shot-sparse-sidecar-251',
                '--suite', $row.suite,
                '--first-seed', $seed,
                '--seeds', 1,
