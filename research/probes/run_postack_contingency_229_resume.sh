#!/usr/bin/env bash
# SCORE-POSTACK-CONTINGENCY-DIVERSIFICATION-229 resumable paired runner.
# Usage: run_postack_contingency_229_resume.sh <binary> <development|holdout> <off|on> <logdir>
set -euo pipefail
...
log="$logdir/SCORE-POSTACK-CONTINGENCY-DIVERSIFICATION-229-$split-causal-$side.log"
if [[ ! -f "$log" ]]; then
    {
        echo "experiment=SCORE-POSTACK-CONTINGENCY-DIVERSIFICATION-229"
...
run_tier() {
    ...
    for ((offset=0; offset<count; ++offset)); do
        ...
        "$binary" \
            ...
            --post-ack-contingency-diversification "$flag" >> "$log" 2>&1
    done
}

if [[ "$split" == development ]]; then
    run_tier stratified-easy 5300000 8 4 12 14
    run_tier stratified-medium 5301000 8 4 12 18
    run_tier stratified-hard 5302000 20 6 18 24
    run_tier stratified-very-hard 5303000 24 8 18 24 30
else
    run_tier stratified-easy 5310000 16 4 12 14
    run_tier stratified-medium 5311000 16 4 12 18
    run_tier stratified-hard 5312000 36 6 18 24
    run_tier stratified-very-hard 5313000 40 8 18 24 30
fi
