#!/usr/bin/env bash
# SCORE-PROOF-PROTECTED-CONTINUATION-232-v2 resumable paired runner.
# Usage: run_proof_protected_continuation_232_v2_resume.sh <binary> <development-control|development|holdout> <off|on> <logdir>
set -euo pipefail
...
mkdir -p "$logdir"
log="$logdir/SCORE-PROOF-PROTECTED-CONTINUATION-232-v2-$split-causal-$side.log"
if [[ ! -f "$log" ]]; then
    {
        echo "experiment=SCORE-PROOF-PROTECTED-CONTINUATION-232-v2"
...
if [[ "$split" == development-control ]]; then
    run_tier stratified-easy 5380000 4 4 5000 12 14
    run_tier stratified-medium 5381000 4 4 5000 12 18
    run_tier stratified-hard 5382000 8 6 5000 18 24
    run_tier stratified-very-hard 5383000 8 8 5000 18 24 30
elif [[ "$split" == development ]]; then
    run_tier stratified-easy 5384000 8 4 15000 12 14
    run_tier stratified-medium 5385000 8 4 15000 12 18
    run_tier stratified-hard 5386000 20 6 15000 18 24
    run_tier stratified-very-hard 5387000 24 8 15000 18 24 30
else
    run_tier stratified-easy 5390000 16 4 15000 12 14
    run_tier stratified-medium 5391000 16 4 15000 12 18
    run_tier stratified-hard 5392000 36 6 15000 18 24
    run_tier stratified-very-hard 5393000 40 8 15000 18 24 30
fi
