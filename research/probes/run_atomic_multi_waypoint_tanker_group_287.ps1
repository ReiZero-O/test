param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-ATOMIC-MULTI-WAYPOINT-TANKER-GROUP-287.csv',
    [string]$Binary = 'build-research-287-msvc/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = '',
    [switch]$ValidateOnly,
    [switch]$Resume
...
$expectedManifestSha256 = '9B688B10E51E0C1260E5CA548F32099391C4A0A6A624D49B60B0E1361C91B3AF'
$suiteAgents = @{
    'general' = 4
    'multiteam-12' = 4
    'multiteam-16' = 6
...
foreach ($row in $allRows) {
    if ($row.experiment_id -ne 'SCORE-ATOMIC-MULTI-WAYPOINT-TANKER-GROUP-287' -or
        $row.split -notin @('development', 'holdout') -or
        -not $suiteAgents.ContainsKey($row.suite)) {
        throw "invalid manifest row: $($row.experiment_id)/$($row.split)/$($row.suite)"
    }
    $agents = [int]$suiteAgents[$row.suite]
    $roleMask = [uint32]$row.role_mask
    $spots = [int]$row.spot_count
    if ([int]$row.count -le 0 -or [int]$row.budget_ms -ne 3375 -or
        [int]$row.role_ms -ne 5000 -or [int]$row.players -notin @(8, 9, 10) -or
        $row.role_mode -ne 'fixed' -or
        $row.fuel_profile -notin @('low', 'default', 'high', 'generated') -or
        -not (Test-SingleBit $roleMask) -or
        $roleMask -ge ([uint32]1 -shl $agents)) {
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-ATOMIC-MULTI-WAYPOINT-TANKER-GROUP-287-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-ATOMIC-MULTI-WAYPOINT-TANKER-GROUP-287,phase=${Phase},expected_cases=${expectedCases},expected_results=${expectedResults}"
...
foreach ($line in Get-Content -LiteralPath $outputPath) {
    ...
    } elseif ($line -match '^result,version=(parent|candidate)-287,track=[^,]+,suite=([^,]+),.*seed=([0-9]+),') {
...
if ($runComplete) {
    if ($completed.Count -ne $expectedCases) { throw 'invalid completed run' }
...
foreach ($row in @($allRows | Where-Object { $_.split -eq $Phase })) {
    for ($offset = 0; $offset -lt [int]$row.count; ++$offset) {
        $seed = [int64]$row.first_seed + $offset
        $key = "$($row.suite)|${seed}"
        if ($completed.Contains($key)) { continue }
        $common = @(
            '--track', 'atomic-multi-waypoint-tanker-group-287',
            '--suite', $row.suite,
            '--first-seed', $seed,
            '--seeds', 1,
        ...
        foreach ($arm in $arms) {
            $arguments = @('--version', "$($arm[0])-287") + $common +
                @('--multi-waypoint-tanker-provider', $arm[1])
