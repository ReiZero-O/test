param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-TERMINAL-STOCK-MARGINAL-RESERVOIR-258.csv',
    [string]$Binary = 'build-research-258-msvc/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = '19F9A528DB330BEF93A801CA381D01C9E506AF704AE1EED5D1A7CD51792BE9E9',
    [switch]$ValidateOnly,
    [switch]$Resume
)
...
$expectedManifestSha256 = '0408971890500E70D4ABD2E909345C6F212A4DB946EB9555C35D11FBEC7D372A'
$suiteSides = @{
    'multiteam-12' = 12
    'multiteam-16' = 16
    'multiteam-24' = 24
...
foreach ($row in $allRows) {
    if ($row.experiment_id -ne 'SCORE-TERMINAL-STOCK-MARGINAL-RESERVOIR-258' -or
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-TERMINAL-STOCK-MARGINAL-RESERVOIR-258-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-TERMINAL-STOCK-MARGINAL-RESERVOIR-258,phase=${Phase},expected_results=${expectedResults}"
...
foreach ($line in Get-Content -LiteralPath $outputPath) {
    ...
    } elseif ($line -match '^result,version=causal-258,track=[^,]+,suite=([^,]+),.*seed=([0-9]+),') {
