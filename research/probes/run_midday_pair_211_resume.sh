#!/usr/bin/env bash
# SCORE-MIDDAY-PAIR-EXCHANGE-211 development resume wrapper. Same-binary
# causal A/B: both sides run the accepted production configuration
# (terminal pair 1, midday chain 1); the sides differ only in --midday-pair 0|1.
...
# Usage: run_midday_pair_211_resume.sh <binary> <side:off|on> <logdir>
set -euo pipefail
binary="$1"
side="$2"
logdir="$3"
...
log="$logdir/SCORE-MIDDAY-PAIR-EXCHANGE-211-development-causal-$side.log"

if [ ! -f "$log" ]; then
    {
        echo "experiment=SCORE-MIDDAY-PAIR-EXCHANGE-211"
        echo "split=development-causal"
        echo "side=$side"
        echo "manifest_sha256=1F3A993411ADA1EF8C9E2B8D43F69D254B02AF615AD386F59838E42FD2EA5AFB"
    } > "$log"
fi
...
run_tier() {
    ...
    for ((offset=0; offset<count; ++offset)); do
        ...
        "$binary" \
            ...
            --terminal-pair 1 \
            --midday-chain 1 \
            --midday-pair "$pairFlag" >> "$log" 2>&1
    done
}

run_tier stratified-easy 4940000 8 4 12 14
run_tier stratified-medium 4941000 8 4 12 18
run_tier stratified-hard 4942000 20 6 18 24
run_tier stratified-very-hard 4943000 24 8 18 24 30
