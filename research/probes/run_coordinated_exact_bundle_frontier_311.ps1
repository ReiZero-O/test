param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-COORDINATED-EXACT-BUNDLE-FRONTIER-311.csv',
    [string]$Binary = 'build-research-311-msvc/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = 'D0B5F3C9035C102E24E06B459DE322A15C78C766790824373E6BDB898758956A',
    [switch]$ValidateOnly,
    [switch]$Resume
)
...
$expectedManifestSha256 = '588072A95C0C7FF15CE7E61C257194445C162EC781A2256BB0E38A95FC57395F'
$suiteSides = @{
    'multiteam-12' = 12
    'multiteam-16' = 16
    'multiteam-24' = 24
...
foreach ($row in $allRows) {
    if ($row.experiment_id -ne 'SCORE-COORDINATED-EXACT-BUNDLE-FRONTIER-311' -or
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-COORDINATED-EXACT-BUNDLE-FRONTIER-311-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-COORDINATED-EXACT-BUNDLE-FRONTIER-311,phase=${Phase},expected_cases=${expectedCases},expected_results=${expectedResults}"
...
foreach ($line in Get-Content -LiteralPath $outputPath) {
    ...
    } elseif ($line -match '^result,version=(parent|candidate)-311,track=[^,]+,suite=([^,]+),.*seed=([0-9]+),') {
...
foreach ($row in $rows) {
    for ($offset = 0; $offset -lt [int]$row.count; ++$offset) {
        ...
        $parentArguments = @(
            '--version', 'parent-311',
            '--track', 'coordinated-exact-bundle-frontier-311-off',
            '--coordinated-exact-bundles', 1
        ) + $common
        $candidateArguments = @(
            '--version', 'candidate-311',
            '--track', 'coordinated-exact-bundle-frontier-311-on',
            '--coordinated-exact-bundles', 4
        ) + $common
