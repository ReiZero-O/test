param(
    ...
    [string]$OutputPath = 'research/evidence/SCORE-TERMINAL-COORDINATE-ASCENT-191-development.log'
)

$ErrorActionPreference = 'Stop'
$root = (Resolve-Path (Join-Path $PSScriptRoot '..\..')).Path
$manifest = Join-Path $root 'research\holdouts\SCORE-TERMINAL-COORDINATE-ASCENT-191.csv'
...
foreach ($row in $rows) {
    ...
    for ($offset = 0; $offset -lt $count; ++$offset) {
        ...
        $arguments = @(
            '--version', 'dev191-terminal-coordinate',
            '--track', $Split,
