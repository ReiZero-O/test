#!/usr/bin/env bash
# SCORE-POSTACK-PROOF-WITNESS-230 resumable paired runner.
# Usage: run_postack_proof_witness_230_resume.sh <binary> <development|holdout> <off|on> <logdir>
set -euo pipefail
...
side="$3"
logdir="$4"
manifest_sha256=741944ED1C48FFF5BEBB3E3EF7D33BA82E77959136CC25D5592F97639544D8B3
...
log="$logdir/SCORE-POSTACK-PROOF-WITNESS-230-$split-causal-$side.log"
if [[ ! -f "$log" ]]; then
    {
        echo "experiment=SCORE-POSTACK-PROOF-WITNESS-230"
...
run_tier() {
    ...
    for ((offset=0; offset<count; ++offset)); do
        ...
        "$binary" \
            ...
            --post-ack-proof-witness-adoption "$flag" >> "$log" 2>&1
    done
}

if [[ "$split" == development ]]; then
    run_tier stratified-easy 5320000 8 4 12 14
    run_tier stratified-medium 5321000 8 4 12 18
    run_tier stratified-hard 5322000 20 6 18 24
    run_tier stratified-very-hard 5323000 24 8 18 24 30
else
    run_tier stratified-easy 5330000 16 4 12 14
    run_tier stratified-medium 5331000 16 4 12 18
    run_tier stratified-hard 5332000 36 6 18 24
    run_tier stratified-very-hard 5333000 40 8 18 24 30
fi
