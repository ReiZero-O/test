...
def main() -> None:
    ...
    for key in sorted(metadata):
        ...
        parent = slash_score(row["terminal_marginal_parent"])
        actual = actual_score(row)
        if slash_score(row["terminal_marginal_refined"]) != actual:
    ...
    for record in lane_5000:
        ...
        if (
            ...
            or int(row.get("terminal_marginal_routes", "0")) != 0
            or int(row.get("terminal_marginal_plans", "0")) != 0
            or int(row.get("terminal_marginal_valid", "0")) != 0
            or int(row.get("terminal_marginal_acceptances", "0")) != 0
            or int(row.get("terminal_marginal_rounds", "0")) != 0
            or int(row.get("terminal_marginal_deadline", "0")) != 0
            or int(row.get("terminal_marginal_failure", "0")) != 0
        ):
    ...
    takeover_records = [
        record
        for record in records
        if int(record["row"].get("terminal_marginal_acceptances", "0")) > 0
    ]
    ...
    longer_gain = any(
        record["outcome"] == "win" and record["meta"]["public_window_ms"] != "5000"
        for record in records
    )
    gate = {
        ...
        "at_least_one_strict_longer_window_gain": longer_gain,
    ...
    summary = {
        ...
        "candidate_work": {
            "routes": summed("terminal_marginal_routes"),
            "plans": summed("terminal_marginal_plans"),
            "valid": summed("terminal_marginal_valid"),
            "acceptances": summed("terminal_marginal_acceptances"),
            "rounds": summed("terminal_marginal_rounds"),
            "deadline_rollbacks": summed("terminal_marginal_deadline"),
            "failures": summed("terminal_marginal_failure"),
        },
        "takeover_breadth": {
