param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-CAUSAL-PERSISTENT-AGENT-PERMUTATION-254.csv',
    [string]$Binary = 'build-research-254-msvc/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = '3260C73B94D088089760B425B386AAFA0F8A38B8F1B28DEF20D6F44C3CC16299',
    [switch]$ValidateOnly,
    [switch]$Resume
)
...
$expectedManifestSha256 = '68B09332375155D3719543B0D4DB1D8604D18C8475E3D70A67A799ED4AD934A3'
$suiteSides = @{
    'multiteam-12' = 12
    'multiteam-16' = 16
    'multiteam-24' = 24
...
foreach ($row in $allRows) {
    if ($row.experiment_id -ne 'SCORE-CAUSAL-PERSISTENT-AGENT-PERMUTATION-254' -or
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-CAUSAL-PERSISTENT-AGENT-PERMUTATION-254-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-CAUSAL-PERSISTENT-AGENT-PERMUTATION-254,phase=${Phase},expected_results=${expectedResults}"
...
foreach ($line in Get-Content -LiteralPath $outputPath) {
    ...
    } elseif ($line -match '^result,version=causal-254,track=[^,]+,suite=([^,]+),.*seed=([0-9]+),') {
...
foreach ($row in $rows) {
    for ($offset = 0; $offset -lt [int]$row.count; ++$offset) {
        ...
        $arguments = @(
            '--version', 'causal-254',
            '--track', 'causal-persistent-agent-permutation-254',
            '--suite', $row.suite,
            '--first-seed', $seed,
            '--seeds', 1,
