param(
    ...
    [string]$Manifest = 'research/holdouts/SCORE-MULTI-WAYPOINT-TANKER-PROVIDER-286.csv',
    [string]$Binary = 'build-research-286-msvc/udonshield_historical_tournament.exe',
    [string]$Output = '',
    [string]$BinarySha256 = '',
    [switch]$ValidateOnly,
    [switch]$Resume
...
$expectedManifestSha256 = 'C2BA2E7518402FA204FADB73138243FC29D33DFB0161AD6DB1A444B2B748B582'
$suiteAgents = @{
    'general' = 4
    'multiteam-12' = 4
    'multiteam-16' = 6
...
foreach ($row in $allRows) {
    if ($row.experiment_id -ne 'SCORE-MULTI-WAYPOINT-TANKER-PROVIDER-286' -or
        $row.split -notin @('development', 'holdout')) {
        throw "invalid experiment/split: $($row.experiment_id)/$($row.split)"
    }
    if (-not $suiteAgents.ContainsKey($row.suite)) {
...
if ([string]::IsNullOrWhiteSpace($Output)) {
    $Output = "research/evidence/SCORE-MULTI-WAYPOINT-TANKER-PROVIDER-286-${Phase}.log"
}
...
if (-not (Test-Path -LiteralPath $outputPath)) {
    @(
        "run_begin,experiment=SCORE-MULTI-WAYPOINT-TANKER-PROVIDER-286,phase=${Phase},expected_cases=${expectedCases},expected_results=${expectedResults}"
...
foreach ($line in Get-Content -LiteralPath $outputPath) {
    ...
    } elseif ($line -match '^result,version=(parent|candidate)-286,track=[^,]+,suite=([^,]+),.*seed=([0-9]+),') {
...
foreach ($row in $rows) {
    for ($offset = 0; $offset -lt [int]$row.count; ++$offset) {
        ...
        $common = @(
            '--track', 'multi-waypoint-tanker-provider-286',
            '--suite', $row.suite,
            '--first-seed', $seed,
            '--seeds', 1,
        ...
        foreach ($arm in $arms) {
            $arguments = @('--version', "$($arm[0])-286") + $common +
